...
1. **On the machine with the project** (the one with Grok signed in): unzip *that machine's* zip, then
   ...
   It starts the agent, puts `tailscale serve` in front, and prints `https://<host>.<tailnet>.ts.net/`. Leave it running.
...
Tailscale identity is the credential on that path; no token is needed. What that gate does and does not check is under [Tailscale](#tailscale-another-machine). **Never use `tailscale funnel`** — that publishes the agent to the internet.
